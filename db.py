...
class Database:
    ...
    def save_starbucks_record(self, gender, age, purchase):
            """survey 기록을 데이터베이스에 저장"""
            try:
                ...
                with self.connection.cursor() as cursor:
                    query = """
                    INSERT into starbucks (gender, age, purchase)
                    VALUES (%s, %s, %s)
                    """
                    cursor.execute(query, (gender, age, purchase))
    ...
    def get_starbucks_records(self, limit=10):
        """최근 survey 기록을 가져옵니다"""
        try:
            if self.connection is None:
                print("데이터베이스 연결이 없습니다.")
